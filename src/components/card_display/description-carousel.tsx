	...
	return (
		<div className='container'>
			...
			<div className='description-carousel'>
				<img src={arrowL} className='arrow left' onClick={() => handleArrowClick('-')} />
				<img className='card' src={cards[currentCard]} alt="" />
				<img src={arrowR} className='arrow right' onClick={() => handleArrowClick('+')} />
			</div>
			<div className="description-section">
			<p className='description'><h2 className='title'>{descriptions[currentCard].name}</h2>{descriptions[currentCard].description}</p>
			</div>
		</div>
	);
}
